...
class Board:
    ...
    #Functions for Heuristic 
    
    def two_match_in_a_row(self, row, startcol):
        for x in range(0, 1):
            if self.board[row][startcol + x] != self.board[row][startcol + x + 1]:
                return False
        return True

    def two_match_in_a_col(self, startrow, col):
        for x in range(0, 1):
            if self.board[startrow + x][col] != self.board[startrow + x + 1][col]:
                return False
        return True

    def two_match_in_ne_diag(self, startrow, startcol):
        for x in range(0, 1):
            if self.board[startrow + x][startcol + x] != self.board[startrow + x + 1][startcol + x + 1]:
                return False
        return True

    def two_match_in_nw_diag(self, startrow, startcol):
        for x in range(0, 1):
            if self.board[startrow + x][startcol - x] != self.board[startrow + x + 1][startcol - x - 1]:
                return False
        return True

    def three_match_in_a_row(self, row, startcol):
        for x in range(0, 2):
            if self.board[row][startcol + x] != self.board[row][startcol + x + 1]:
                return False
        return True

    def three_match_in_a_col(self, startrow, col):
        for x in range(0, 2):
            if self.board[startrow + x][col] != self.board[startrow + x + 1][col]:
                return False
        return True

    def three_match_in_ne_diag(self, startrow, startcol):
        for x in range(0, 2):
            if self.board[startrow + x][startcol + x] != self.board[startrow + x + 1][startcol + x + 1]:
                return False
        return True

    def three_match_in_nw_diag(self, startrow, startcol):
        for x in range(0, 2):
            if self.board[startrow + x][startcol - x] != self.board[startrow + x + 1][startcol - x - 1]:
                return False
        return True
